...
$(()=>{
    ...
    ************************************************/
    // 1. 햄버거 버튼
    $(".hbtn").click(()=>{ // 자기 자신 this 쓰지 않구 부를거니까 그냥 애로우
        // 변경 내용 : GNB 박스 슬라이딩 보임 / 숨김
        // slideToggle() -> 슬라이드 업 다운!
        // 대상: 모바일 GNB
        // $(''). 주인공이 누군지 몰라도 이렇게 써서 일단 확인 가능
        // $('').slideToggle(400);
        $('#mobx').slideToggle(400,resetH); /* 이부분 영상 다시 봐야할 듯,,, 1번째 시간 */
    ...
    // 2. 검색 버튼
    $(".sbtn").click(()=>{ // 자기 자신 this 쓰지 않구 부를거니까 그냥 애로우
        // 변경 내용 : GNB 박스 슬라이딩 보임 / 숨김
        // slideToggle() -> 슬라이드 업 다운!
        // 대상: 모바일 GNB
    ...
    const resetH = ()=>{
        // #mobx의 높이값을 동적으로 생성함
        console.log(
            '.top의 높이값: ',$('.top').innerHeight(), /* height는 패딩등 포함안된 높이값 jquery 설명서 보길!! 017번에 있음! */
            '\n.mos의 높이값: ',$('.mos').css('display')
        );
        ...
        // // 변경할 높이
        // let Hval = 
        // $('.top').innerHeight() + /* 이 이너하이트 값에 더하기 */
        // $('.mos').css('display') === 'none'? /* mos css가 display none이냐? */
        // 0:$('.mos').innerHeight(); /* 그럼 0이고 아니면 이너 하이트 값을 넣어줘라 */
        
        // innerHeight()는 패딩포함 높이
        // height()는 순수 높이 (컨텐츠만)
        // 017문서 참조

        console.log('.top 높이: ', $('.top').innerHeight());

        console.log('.mos 높이: ', $('.mos').css('display')=== 'none'? 0:$('.mos').innerHeight());
