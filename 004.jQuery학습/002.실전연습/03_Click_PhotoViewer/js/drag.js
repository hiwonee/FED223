...
$(() => {
    ...
    $(".abtn").click(function () {
        ...
        if (isR) { // 오른쪽 버튼
            ...
            slide.prepend(slide.find("li").last())
                // 동시에 left값 -100%
                // .css({left:"-100%"})
                // 부모박스 sbx의 가로크기 마이너스는 -100%와 동일..
                // 보정값으로 드래그 이동시 위치 맞추기
    ...
    ************************************************************/
    indic.click(function () {
        ...
        if (diff > 0) { /// 양수는 오른쪽에서 들어옴
            ...
            //////////////// 예외처리! //////////////////////////
            // 첫번째 슬라이드에서 마지막 슬라이드 클릭시 차이수가! 3이고! 블릿 순번이 3번일때!!!
            // 아니 걍 차이수가 3이면 예외처리~~ 3이 더 나오지 않잖아??
            // 그러나 배너 개수가 변경될 수 있으므로,,, // (전체 개수 - 1)로 계산시키는게 안전빵 indic ,, 블릿 개수랑 같으니까 블릿으로,,
            // 블릿 전체수 -1
    ...
    *****************************************************/
    ...
    slide.on("dragstop", function () {
        ...
        // let spos = slide.offset().left;
        // console.log("슬위:", spos); /* 끌다가 손 딱 놓았을 때 값이 나오겠지 */
        // 화면 맨 왼쪽부터의 거리로 나올 것임.
        // offset은 절대 위치값임.. 상대위치값을 배울거야
        // offset().left는 보이는 화면 왼쪽선 기준함
        // position().left는 싸고있는 포지션이 있는 부모박스를 기준함
        let spos = slide.position().left;
        console.log("슬위: ", spos);

        // // 2. 기준값 계산
        // // 중심값
        // let setval = slide.width();
        // console.log("기준값: ", setval);

        // 3. 위치값에 따라 슬라이드 이동하기
        // 기준: 0을 중심으로 50px과 -50px 이거 안돼 왜냐면 기준값이 필요하고,, 위드스 600도 있구,,
        // 기준값의 90%는 setval*0.9
        // 기준값의 100%는 setval*1.1

        // 3-1. 50px보다 클 때 왼쪽에서 들어옴! 왼쪽 버튼 클릭되게 함.
        if (spos > 50) {
            // 드래그한 만큼 위치값 보정하기
            // tval에 보정!
            tval = -spos;
            $(".lb").trigger("click"); /* trigger 클릭 강제 */
            setTimeout(() => {
                cover.hide(); /// 커버해제
            }, aniT);
        }

        // 3-2. -50px보다 작을 때 오른쪽에서 들어옴!!
        else if (spos < -50) {
            $(".rb").trigger("click");
            setTimeout(() => {
                cover.hide();
